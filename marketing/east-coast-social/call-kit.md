# East Coast Social — cold-call kit

_The phone channel: for businesses with no social page to DM, and DM ghosts
after the follow-up window. You call, personally — that's the brand.
Log every outcome in prospects.md after the call._

## When to call
- **Shops & services:** 10:00–11:30 AM (open, not yet slammed)
- **Restaurants & cafés:** 2:00–4:00 PM ONLY (never during a meal rush)
- Avoid Monday mornings everywhere. Friday afternoons are surprisingly good.

## The 30-second opener (sounds human, not scripted)

> Hi — is this the owner? My name's Matthew, I'm calling from Memramcook —
> so, basically a neighbour. I'll keep it to twenty seconds: I run a little
> local service that makes a business's Facebook page post every single day,
> automatically — you'd approve the look once and never touch it again.
> I noticed [you don't have a page / your page's been quiet since MONTH],
> and honestly that's money left on the table. Setup's free, it's $79 a
> month, and I'll build your first week of sample posts for nothing so you
> can see it with your own shop. Worth a coffee sometime this week?

**Key beats:** neighbour → 20 seconds → what it does → free to try → coffee.
If they're busy: "When's a better time? I'll call back — 30 seconds, promise."

## Voicemail version (15 seconds)

> Hi, this is Matthew from Memramcook. I help local businesses keep their
> Facebook pages posting every day, automatically — free setup, and the
> first week of sample posts is free to look at. I'll try you another time,
> or you can text me at (506) 889-9737. Thanks!

## Objection cheat sheet

| They say | You say |
|---|---|
| "Too busy right now" | "That's literally why this exists — it runs so you don't have to. When's a good 20 minutes this week?" |
| "My niece/kid does our page" | "Perfect — keep them! This runs underneath: it never misses a day, and they can post on top whenever they want. It makes them look better, not replaced." |
| "How much?" | "Free to set up, $79 a month, cancel any month. For scale — agencies charge a thousand-plus for the same daily posting." |
| "Is this one of those AI things?" | "The posting engine is automated, yes — that's why it's $79 and not $1,000. But you're talking to the human who runs it, and I'm 10 minutes away. You approve everything before it starts." |
| "Send me something" | "Will do — what's the best email? I'll send the one-pager and a link to my own store running on it, live." (→ send flyer PDF + findhotstuff.com/automation from ceohotstuff@yahoo.com) |
| "Not interested" | "No worries at all — thanks for the twenty seconds. If the page ever goes quiet and it bugs you, I'm around." (Mark ❌ in ledger, never call again.) |

## Call sheet — round 1

| Business | Town | Phone | Why calling | Outcome |
|---|---|---|---|---|
| Carlos Barber Shop | Sackville (22 Lansdowne St) | (902) 297-3433 | No social page at all — needs it most | 08-04 voicemail (touch 1) — call back ~08-07 |
| LeBlancs Meat Market | Memramcook (32 La Montain Rd) | (506) 758-2222 | Page messaging disabled; email drafted; French-first greeting is a nice touch: "Bonjour! Je peux continuer en anglais?" | |
| Boulangerie Tony | Dieppe (50 Rue du Marché) | (506) 388-2253 | Has FB but low activity; bakery = perfect daily-post client | 08-04 no answer, no voicemail — appears open Wed-Fri only. Call Wed 08-05 ~10:30 AM |
| A & M Auto Repair | Riverview (569 Pine Glen Rd — listing says Riverview, not Dieppe) | (506) 387-4151 | No social seen; "most honest garage" reviews = great content to automate | 08-04 closed (vacation). 08-11 AM no answer, voicemail left (touch 1). One callback ~Fri 08-14 or Mon, then rest |
| CAVOK Brewing Co. | Dieppe (250 Dieppe Blvd) | (506) 961-5835 | No social seen; breweries thrive on daily taps/events posts | 08-04 🔥 HOT: staff gave owner's cell, owner answered, asked for TEXT — sent free-week-of-samples offer. If quiet, bump ~08-06 |
| Norm's Pizza | Dieppe (4-438 Paul St) | (506) 859-8180 | No social seen; pizza = easiest daily content there is | 08-04 owner out — left contact info with staff (touch 1). Call back ~08-06 in the 2-4 window |

## Call sheet — round 2 (added 08-11; C.C recon done 08-11)

| Business | Town | Phone | Why calling | Outcome |
|---|---|---|---|---|
| La Boulangerie Léger | Shediac (482 Main St) | (506) 532-3137 | 🔥 BEST DATA ON THE SHEET: FB page has 2.2K followers but silent since July 4 — and that last post pulled 50 reactions + 34 shares. Their audience responds; they just stopped posting. Opener writes itself. | 08-12 voicemail (touch 1). 08-14 callback connected — SAID NO (touch 2). RESTING — no further calls unless she reaches out; her pack stays rendered if she ever does. Reason for the no not recorded — if it was price/commitment it counts toward the downsell gate (council 08-14). |
| LeBlanc Restaurant (Ben's) | Memramcook (583 rue Centrale) | (506) 758-2554 | Recon: NO Facebook page at all, yet notable enough that Dominic LeBlanc posted about lunching there with Premier Holt. Village restaurant, pizza menu = daily specials. NOT LeBlancs Meat Market. Hours found via directory: 11:30–8 daily, Fri/Sat till 9 (2021 listing — confirm at door); pack rendered COMPLETE (in repo: engine/samples/leblanc-resto). | 08-13 voicemail (touch 1). 08-14 WALK-IN DONE — printed pack handed over in person (touch 2) 🔥 hottest prospect on the sheet. Follow-up ~Thu 08-20, in person or call: 'any thoughts on the pack? want me to run your free week for real?' |
| Salon Coupe Chic | Memramcook (280 La Vallée Rte) | (506) 758-9390 | ⚠️ DOWNGRADED: no FB page found, and a resident posted in the Memramcook group that the salon isn't answering its phone (no voicemail), asking if it closed. One verify-call at most; expect nothing. | |

_Skipped in research: Culinanny Bakery Shediac (reported closed), Sackville
"902" listings (that's Sackville Nova Scotia — wrong province trap)._

_Rules: max ~5 calls a session, same as DMs. After each call, tell Claude
the outcome → ledger gets updated. A "no" is a complete result — log it and
move on. A voicemail counts as touch one; call back once, days later, then
let it rest._

## Walk-in playbook (hotels, no-page shops, DM ghosts)

**Bring:** 2 dark flyers, 3 business cards, phone with two tabs ready:
findhotstuff.com/automation and the X feed x.com/FindHotsTuff (live proof — TikTok is @find.hotstuff).

**Timing:** mid-morning (10–11) or mid-afternoon (2–4), midweek. For a
hotel, ask the front desk: "Is the GM or whoever handles marketing around?
Twenty seconds, I promise." Get a NAME even if they're out.

**The pitch (Hôtel Shediac / Bistro Blonde version):** "Your hotel page is
actually great — but Bistro Blonde hasn't posted in about two years, and
that's a restaurant in the middle of Shediac's tourist season. I run a
local service that gives it its own daily voice, automatically — free
setup, $79 a month, and I'll build the first week of sample posts free so
you can see it before deciding." Show the store feed scrolling. Hand over
flyer + card.

**The close:** "Who should I email the one-pager to?" — a name + email is
a WIN even without a yes. Log every walk-in outcome in prospects.md.
